...
- Receiver aliases are edited from the dashboard with `n` and persisted under
  `[receiver_names]` in the active TOML configuration.
- `protocol.md` — DMX, priority, management, telemetry, and ACK formats.
- `management-interface.md` — detailed host/transmitter/receiver control-plane
  guide with packet flow diagram.
