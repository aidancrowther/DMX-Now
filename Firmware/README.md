...
- `ReTransmitter/` — receives physical DMX through a receive-only RS-485 input
  and rebroadcasts the normal three-fragment wireless protocol. Production builds
  also publish best-effort retransmitter telemetry; `--telemetry-only` is a
  diagnostic scheduler variant and is not a deployment image.
